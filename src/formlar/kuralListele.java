...
public class kuralListele {
	...
	public static void Listele() {
		dbClasslar.kurallar.kurallarDBList();
		table = Fonksiyonlar.JTableVeriTemizleme(table);
		Object[] obj = DiziDegiskenler.KurallarListMap.values().toArray();
		for (int i = 0; i < obj.length; i++) {
			dbClasslar.kurallar kurallar = (dbClasslar.kurallar) obj[i];
			table = Fonksiyonlar.JTableVeriEkleme(table, kurallar.kurallarGetModel());
		}
	}
	...
	private static void AramaYap(String aranan_veri) {
		...
		for (int i = 0; i < obj.length; i++) {
			dbClasslar.kurallar kurallar = (dbClasslar.kurallar) obj[i];
			if (kurallar.getKur_ad().toLowerCase().contains(aranan_veri)) {
				table = Fonksiyonlar.JTableVeriEkleme(table, kurallar.kurallarGetModel());
			}
		}
	}
	...
	private ActionListener Ekle = new ActionListener() {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			new kural("Ekleme",-1).formAl().setVisible(true);
		}
	};
		...
		btnDzenle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (i >= 0) {
					...
					new kural("Güncelleme", id).formAl().setVisible(true);
				} else {
		...
		btnSil.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (i >= 0) {
					...
					dbClasslar.kurallar kurallar = (dbClasslar.kurallar) DiziDegiskenler.KurallarListMap.get(id);
					kurallar.kurallarDBSil();
					DiziDegiskenler.KurallarListMap.remove(id);
				} else {
	...
	private void Yükle() {
		...
		frame = Fonksiyonlar.AnaFormOlustur(100, 100, 1300, 500, "Kural Listeleme");

		table = Fonksiyonlar.JTableOlustur(DiziDegiskenler.KurallarListeleme);
