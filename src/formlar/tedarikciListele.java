...
public class tedarikciListele {

	private static JTextField txtAra = null;
	private JFrame frame;
	private static JTable table;

	public tedarikciListele() {
	...
	public static void Listele() {
		...
		for (int i = 0; i < obj.length; i++) {
			dbClasslar.tedarikci tedarikci = (dbClasslar.tedarikci) obj[i];
			if (tedarikci.getSilindimi().equals("h")) {
				table = Fonksiyonlar.JTableVeriEkleme(table, tedarikci.tedarikciGetModel());
			}
		}
	}
	...
	private static void AramaYap(String aranan_veri) {
		...
		for (int i = 0; i < obj.length; i++) {
			dbClasslar.tedarikci tedarikci = (dbClasslar.tedarikci) obj[i];
			if (tedarikci.getSilindimi().equals("h")) {
				if (tedarikci.getT_ad().toLowerCase().contains(aranan_veri)
						|| tedarikci.getT_email().toLowerCase().contains(aranan_veri)
						|| tedarikci.getT_telefon().toLowerCase().contains(aranan_veri)) {
					table = Fonksiyonlar.JTableVeriEkleme(table, tedarikci.tedarikciGetModel());
				}
			}
		}
	...
	private ActionListener Ekle = new ActionListener() {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			new tedarikci("Ekleme", -1).formAl().setVisible(true);
		}
	};
		...
		btnDzenle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (i >= 0) {
					...
					new tedarikci("Güncelleme", id).formAl().setVisible(true);
				} else {
		...
		btnSil.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (i >= 0) {
					...
					dbClasslar.tedarikci tedarikci = (dbClasslar.tedarikci) DiziDegiskenler.TedarikciListMap.get(id);
					tedarikci.tedarikciDBSil();
					DiziDegiskenler.TedarikciListMap.remove(id);
				} else {
	...
	private void Yükle() {
		...
		frame = Fonksiyonlar.AnaFormOlustur(100, 100, 1300, 500, "Tedarikci Listeleme");

		table = Fonksiyonlar.JTableOlustur(DiziDegiskenler.TedarikciListeleme);
